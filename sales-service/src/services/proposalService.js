/* === D:\SmartIASystems\sales-service\src\services\proposalService.js === */
...
/** Normaliza “ativo” a partir de formatos: boolean, string, array ou objeto */
function isActiveLike(val) {
  ...
  if (Array.isArray(val)) {
    const first = val[0];
    // pode ser ["active"] ou [{ status: "active" }, ...]
...
/**
 * Compat. Agora recebe arrays buyerId[] e sellerId[].
 */
export async function createProposal({ buyerId, sellerId, products, ownerAuthId, bearer }) {
  const PRODUCT_URL = process.env.APP_PRODUCT_SERVICE_URL;
  const CONTACT_URL = process.env.APP_CONTACT_SERVICE_URL;
  const SELLER_URL  = process.env.APP_SELLER_SERVICE_URL;
  if (!PRODUCT_URL || !CONTACT_URL || !SELLER_URL) {
    throw new AppError('service_urls_not_configured', 500, 'Missing APP_PRODUCT_SERVICE_URL or APP_CONTACT_SERVICE_URL or APP_SELLER_SERVICE_URL');
  }
  ...
  const doc = await Proposal.create({
    ...
    sellerId,
    products,
    status: [{ status: 'created', statusStartedAt: now }],
