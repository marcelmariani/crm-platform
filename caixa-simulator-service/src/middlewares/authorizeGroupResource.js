...
export function authorizeGroupResource(resourceName, requiredPerm) {
  return async (req, res, next) => {
    try {
      ...
      const base = (process.env.JWT_SERVICE_URL || '').replace(/\/+$/,''); // ex: http://auth-service:3000/v1
      const url  = new URL(`grants/effective?groupId=${encodeURIComponent(req.user.group)}&resourceName=${encodeURIComponent(resourceName)}`, base + '/').toString();
      ...
      return res.status(502).json({ message: 'Auth-service indisponível', detail, status });
    }
  };
}
